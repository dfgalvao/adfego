...
class ProgressesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create progress" do
    assert_difference('Progress.count') do
      post progresses_url, params: { progress: { body: @progress.body, preach_id: @progress.preach_id, publication: @progress.publication, status: @progress.status, subject: @progress.subject } }
    end
  ...
  test "should update progress" do
    patch progress_url(@progress), params: { progress: { body: @progress.body, preach_id: @progress.preach_id, publication: @progress.publication, status: @progress.status, subject: @progress.subject } }
